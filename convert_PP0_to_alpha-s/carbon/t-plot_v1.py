...
t_obserbed = fitted_curve(pp0_obserbed)*6.96/4.34*0.354

#index = idx_of_the_nearest(as_obserbed, 0.5)
#print(index)
#print(df.iloc[index,1])
#x = np.linspace(0, max(as_obserbed), 100)*6.96/4.34*0.354
#y = df.iloc[index,1]*2 * x
#s = df.iloc[index,1]*2 * 2.715517

plt.plot(t_obserbed, cm3STP_obserbed, c="red", label="obserbed")
...
plt.xlim(0, as_max*6.96/4.34*0.354)
