...
dft.T.to_csv("./plot/t-plot_NGCB_std_results.csv")

dfmpv = pd.DataFrame([btx,b_t_fitted_data])
dfmpv.index = ['t[nm]', 'dV[cm3(STP)/g]']
dfmpv.T.to_csv("./plot/MP_dV_NGCB_std_results.csv")

dfmps = pd.DataFrame([ddtx,dd_t_fitted_data])
dfmps.index = ['t[nm]', 'dS[m2/g]']
dfmps.T.to_csv("./plot/MP_dS_NGCB_std_results.csv")
# ***************************************